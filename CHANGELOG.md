...
## 2026-05-21 | 分享链接跨设备稳定性修复

### 分享发布

- **便携链接瘦身**：针对当前参考个人网站生成轻量便携快照，把跨设备 hash 链压缩到微信可发送的量级，避免依赖不稳定的服务端临时存储。
- **主链接策略调整**：便携链不超长时优先复制便携链接；超长时才使用已校验正式短链，避免手机跨实例访问短链出现 401/404。
- **发布校验增强**：服务端发布 POST 成功后，会立即 GET 校验短链可读；校验失败时保留便携链接兜底。
- **诊断区文案区分**：主卡片区分跨设备链接、短链接和长链接兜底，便于用户判断复制的是哪一种链接。

### 验证结果

- `npm test -- src/lib/share/strategy.test.ts` 通过
- `npm run lint` 通过
- `npm run typecheck` 通过
- `npm run build` 通过

## 2026-05-20 | 个人网站企业级 Agent 项目视觉与叙事更新

### 个人网站

- **项目定位升级**：将“企业级 AI Agent 工作流与知识库平台”调整为“汽车 E/E 全域专家 Agent 协作系统”，突出架构、通信、诊断、软件四类专家 Agent 的协作边界。
- **配图替换**：新增 `public/project-visuals/agent-ee-expert-system.png`，替换个人网站项目区旧版 `agent-workflow.png`，保留旧图作为历史资产。
- **文案收敛**：弱化通用 RAG/工作流表达，改为围绕工程对象建模、产品线上下文、变更影响链、设计依据维护、方案复用与工程追溯展开。
- **一致性兜底**：同步更新备用代码化 Agent 项目图的标题、能力点和指标，避免静态图不可用时回退到旧叙事。
- **信息层级微调**：代表项目顺序改为专家 Agent 项目优先；最新职业经历详情标题补充“国科础石”公司名，强化公司与阶段角色的标题级背书。

### 验证结果

- `npm run lint` 通过
- `npm run typecheck` 通过
- `npm test` 通过（23 个测试文件，94 个测试）
- `npm run build` 通过
- 本地页面与线上部署完成后检查
