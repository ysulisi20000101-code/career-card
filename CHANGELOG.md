...
## 2026-05-21 | 分享链接默认短链化

### 分享发布

- **主链接策略调整**：发布成功后优先复制已校验的正式短链接，避免微信等渠道因 hash 便携链接过长而无法发送。
- **发布校验增强**：服务端发布 POST 成功后，会立即 GET 校验短链可读；校验失败时再退回便携长链接兜底。
- **诊断区文案区分**：主卡片区分“可分享短链接 / 可分享长链接（兜底）”，便携 hash 链保留在发布诊断中作为备用。
