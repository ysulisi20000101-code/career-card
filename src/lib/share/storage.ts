...
function buildCompactPortablePayload(data: ResumeData): ResumeData {
  const timeline = limit(data.timeline, 4).map((node) => ({
    id: node.id,
    company: truncate(node.company || "", 40),
    position: truncate(node.position || "", 60),
    startDate: (node.startDate ?? "").slice(0, 7),
    endDate: (node.endDate ?? "").slice(0, 7),
    description: truncate(node.description || "", 120),
    highlights: limit(node.highlights, 2).map((h) => truncate(h, 70)),
    projects: [],
    skills: limit(node.skills, 5),
    order: node.order ?? 0,
    careerKind: node.careerKind,
    storyTitle: node.storyTitle ? truncate(node.storyTitle, 60) : undefined,
    storyOutcome: node.storyOutcome ? truncate(node.storyOutcome, 80) : undefined,
    evidenceResult: node.evidenceResult ? truncate(node.evidenceResult, 80) : undefined,
    promotionStages: limit(node.promotionStages, 2).map((stage) => ({
      id: stage.id,
      title: truncate(stage.title, 40),
      period: truncate(stage.period, 20),
      teamScale: truncate(stage.teamScale, 12),
      leadershipType: stage.leadershipType,
      responsibility: truncate(stage.responsibility, 90),
      outcome: truncate(stage.outcome, 90),
      reflection: "",
    })),
  }));
  ...
  return {
    profile: {
      ...
      title: truncate(data.profile.title ?? "", 70),
      summary: truncate(data.profile.summary ?? "", 140),
    ...
    timeline,
    skills: limit(data.skills, 10).map((s) => ({
      id: s.id,
    ...
    architecture: limit(data.architecture, 6).map((m) => ({
      id: m.id,
      ...
      position: m.position,
      parentId: m.parentId,
      relatedTimelineIds: limit(m.relatedTimelineIds, 2),
    })),
    education: limit(data.education, 2).map((e) => ({
      id: e.id,
      school: truncate(e.school, 30),
      degree: truncate(e.degree, 30),
      major: truncate(e.major, 30),
      startDate: (e.startDate ?? "").slice(0, 7),
      endDate: (e.endDate ?? "").slice(0, 7),
    })),
    roleUnderstanding: {
      targetRoleTitle: truncate(data.roleUnderstanding?.targetRoleTitle || data.profile.title || "", 50),
      companyContext: data.roleUnderstanding?.companyContext,
      oneLineInterpretation: truncate(data.roleUnderstanding?.oneLineInterpretation || "", 80),
      priorityProblems: [],
