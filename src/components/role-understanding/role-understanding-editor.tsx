...
export function RoleUnderstandingEditor() {
  ...
  const updateRoleUnderstanding = useResumeStore((s) => s.updateRoleUnderstanding);

  if (!resumeData) return null;
  const role = resumeData.roleUnderstanding;

  const updateField = (field: "targetRoleTitle" | "companyContext" | "oneLineInterpretation", value: string) => {
    updateRoleUnderstanding({ ...role, [field]: value });
  };

  const updateProblem = (id: string, key: "problem" | "impact" | "evidence", value: string) => {
  ...
  const updatePlan = (key: "day0To30" | "day31To60" | "day61To90", value: string) => {
    updateRoleUnderstanding({
      ...role,
  ...
  const updateMapping = (id: string, key: "requirement" | "myExperience" | "outcomeEvidence", value: string) => {
    updateRoleUnderstanding({
      ...role,
  ...
  return (
    <div className="space-y-6 p-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">按 A 到 B 到 C 到 D 逐段填写，帮助面试中快速讲清角色匹配。</p>
      </div>
      ...
      <section className="space-y-3 rounded-lg border p-4">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={addMapping}>添加映射</Button>
        </div>
        {role.experienceMappings.map((item) => (
          <div key={item.id} className="space-y-2 rounded-md border p-3">
            ...
            <Input
              ...
              maxLength={200}
              onChange={(e) => updateMapping(item.id, "outcomeEvidence", e.target.value)}
            />
          </div>
        ))}
      </section>
